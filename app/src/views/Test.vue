  ...
  <v-app>
    <v-content>
      <v-toolbar :dark="true" flat>
        ...
        <v-toolbar-items>
          <v-btn text :to="{name : 'index', query : {'u': $route.query.u}}">現行UIに戻る</v-btn>
        </v-toolbar-items>
    ...
    axios.get(u).then(response => {
      ...
      axios.get(image_map_url).then(response => {
       ...
       for(let i = 0; i < params.length; i++){
         windows.push({
           manifestId : params[i].manifest
         })
       }

       this.mirador_path = "https://mirador.cultural.jp/?config="+encodeURIComponent(JSON.stringify(config))
